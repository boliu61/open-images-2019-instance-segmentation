...
def init_detector(config, checkpoint=None, device='cuda:0'):
    ...
    if checkpoint is not None:
        checkpoint = load_checkpoint(model, checkpoint, device)
...
def show_gt(img, result, class_names, score_thr=0.3, out_file=None):
    ...
    Args:
        ...
        out_file (str, optional): If specified, the visualization result will
            be written to the out file instead of shown in a window.
    """
    val_ann = mmcv.load('/media/bo/Elements/open-images/data/mmdet_anno/seg_val_2844_ann.pkl')
    assert len([x for x in val_ann if x['filename'] == os.path.basename(img)]) == 1
    this_ann = [x for x in val_ann if x['filename'] == os.path.basename(img)][0]

    assert isinstance(class_names, (tuple, list))
    img = mmcv.imread(img)

    bbs = this_ann['ann']['bboxes']
    bboxes = np.concatenate((bbs, np.ones(bbs.shape[0]).reshape(-1,1)),axis=1)
    segms = this_ann['ann']['MaskPath']
    for i in range(segms.shape[0]):
        color_mask = np.random.randint(0, 256, (1, 3), dtype=np.uint8)
        mask = mmcv.imread('/media/bo/Elements/open-images/data/val_masks/' + segms[i], 'unchanged')
        mask = (mask > 0).astype('uint8')
        mask = mmcv.imresize(mask, (img.shape[1], img.shape[0])).astype(np.bool)
        img[mask] = img[mask] * 0.5 + color_mask * 0.5
